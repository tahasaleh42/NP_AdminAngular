...
export class MainPlaceComponent implements OnInit {
  ...
  constructor(private cookieService: CookieService , private ApiService:APiService,private progress: NgProgress, private modalService: NgbModal,
    private router:Router ,private notifierService: NotifierService, private route: ActivatedRoute ) {
      ...
      this.permissions = localStorage.getItem("prm_id10");
  ...
  onSubmit(formValue){
     ...
     if (this.Userform.valid) {
       ...
       this.ApiService.CreateMainPlace(formValue).subscribe(
         res => {
  ...
  onUpdate(formValue) {
    ...
    if (this.Userform.valid) {
      this.progressRef.start();
      this.ApiService.UpdateMainPlace(formValue,this.taskid).subscribe(
        res => {
  ...
  updatetask(page, id) {
   ...
   this.ApiService.GetByIdMainPlace(id).subscribe(
     res => {
  ...
  deleteTask(id) {
    ...
    swal.fire({
      ...
      if (result.value) {
        ...
        this.ApiService.DeleteMainPlace(id).subscribe(
          res => {
            this.ngOnInit();
          });
